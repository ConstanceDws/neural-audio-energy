...
class DiffusionModel(nn.Module):
    """
    Base class for diffusion models, which must implement:
        - compression_ratio (property method)
            temporal ratio between data and conditioning
        - neural_pass (method)
            args: data, conditioning, noise_level
    """
    @property
    ...
    def sample_noise_level(self, batch_size):
        ...
        return torch.from_numpy(sampled_sqrt_alph_cum).float().to(device)
    ...
    @torch.no_grad()
    def sample(self, cdt, repeat=1):
        bs, _, t = cdt.shape
        y = [torch.randn(bs, 1, self.compression_ratio * t).to(cdt)]
        for t in range(self.n_step)[::-1]:
            for i in range(repeat):
                y.append(self.inverse_dynamics(y[-1], cdt, t))
        return y
